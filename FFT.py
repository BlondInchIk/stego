...
import os
import time
from PyQt5.QtWidgets import QApplication, QMainWindow, QLabel, QComboBox, QPushButton, QFileDialog, QTextEdit, \
...
class SteganographyApp(QMainWindow):
    ...
    def select_input_file(self):
        ...
        if file_name:
            ...
            self.label_picture.setGeometry(400 + ((400 - self.scaled_pixmap.width()) // 2),
                                           25 + ((400 - self.scaled_pixmap.height()) // 2), 400, 400)
            ...
            self.size_label = QLabel("Max data size: " + str(size(self.pixmap.width() * self.pixmap.height() // 8)),
                                     self)
        ...
        try:
            ...
            if result == 0:
                self.log_text_edit.append(
                    "{} completed successfully in {:.2f} seconds.".format(operation, execution_time))
            else:
    ...
    def show_weight(self):
        ...
        self.size_label = QLabel(
            "Max data size: " + str(size(self.pixmap.width() * self.pixmap.height() // 8 * weight)), self)
        self.size_label.setGeometry(25, 310, 200, 30)
        self.size_label.setWordWrap(True)
        self.size_label.show()
